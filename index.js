...
function loadEnvFile() {
  try {
    const ENV_FILE = ENV_FILES.find(file => fs.existsSync(file));
    if (!ENV_FILE) {
      console.warn('⚠️ .env file not found (environment variables may still be provided by the host)');
      return false;
    }
...
console.log('📍 Path:', ENV_FILES.find(file => fs.existsSync(file)) || 'environment variables');
...
const { startDailyPostScheduler } = require('./dailyTechPost');
...
async function startBot() {
  ...
  // ==========================================================
  // WHATSAPP CHANNEL / NEWSLETTER JID DETECTOR
  // ==========================================================

  // Prints the JID whenever a WhatsApp Channel message/post
  // is received. Channel JIDs normally end with @newsletter.
  const detectedChannelJids = new Set();
    ...
    ({ messages }) => {
      for (const msg of messages || []) {
        ...
        if (jid?.endsWith('@newsletter')) {
          if (!detectedChannelJids.has(jid)) {
            detectedChannelJids.add(jid);

            console.log('');
            console.log('╔════════════════════════════════════════╗');
            console.log('║      📢 CHANNEL JID DETECTED          ║');
            console.log('╚════════════════════════════════════════╝');
            console.log(`📢 Channel JID: ${jid}`);
            console.log('📋 Add to .env as:');
            console.log(`TECH_POST_JID=${jid}`);
            console.log('');
          }
        }
      }
  ...
  sock.ev.on(
    ...
    async update => {
      ...
      if (
        ...
        global.__JINWOO_BOT_NUMBER = sock.user?.id?.split(':')[0] || null;

        setWhatsAppStatus('connected');

        // Start CHRIS-TECH daily technology posts once WhatsApp is connected.
        startDailyPostScheduler(sock);
