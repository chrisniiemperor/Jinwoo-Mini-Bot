...
const isOwner = (sender, sock = null) => {
  ...
  // The WhatsApp account that is paired with this bot is the owner.
  // This makes the bot portable: each person who deploys/pairs it
  // automatically becomes the owner without editing config.js.
  if (sock?.user?.id) {
    const botJid =
  ...
  // Keep config.js as a legacy fallback for installations where the
  // socket identity is not available yet.
  const owners =
...
const handleMessage = async (
  ...
  try {
    ...
    dashboardStats.recordMessage({ sender, isGroup, groupId: isGroup ? from : null, groupName: groupMetadata?.subject || null });
    ...
    dashboardStats.recordCommand({ name: commandName, sender, isGroup, groupId: isGroup ? from : null, groupName: groupMetadata?.subject || null, category: command?.category });
